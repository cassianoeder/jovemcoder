...
const AvailableClasses = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const [classesRes, requestsRes, enrollmentsRes] = await Promise.all([supabase.from('classes').select('*, courses(title)').eq('is_public', true).order('name'), supabase.from('enrollment_requests').select('id, class_id, status'), supabase.from('enrollments').select('id, class_id, status')]);
  ...
  const filteredClasses = classes.filter(cls => cls.name.toLowerCase().includes(search.toLowerCase()) || cls.description?.toLowerCase().includes(search.toLowerCase()));
  if (loading) {
  ...
  return <div className="min-h-screen bg-background dark">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {filteredClasses.length === 0 ? <Card className="glass border-border/50">
          ...
          return <Card key={cls.id} className="glass border-border/50 hover:border-primary/30 transition-colors">
                  <CardHeader className="pb-3">
                    <div className="flex items-center gap-2 mb-2">
                      {cls.is_public ? <Globe className="w-4 h-4 bg-success-foreground text-success-foreground" /> : <Lock className="w-4 h-4 text-warning" />}
                      ...
                      {cls.courses && <Badge variant="secondary" className="text-secondary-foreground bg-secondary">
                          <BookOpen className="w-3 h-3 mr-1" />
                          {cls.courses.title}
                        </Badge>}
                    </div>
                    <CardTitle className="text-lg">{cls.name}</CardTitle>
                    {cls.description && <CardDescription className="line-clamp-2">{cls.description}</CardDescription>}
                  </CardHeader>
                  <CardContent>
                    {status === 'enrolled' ? <Link to={`/student/class/${cls.id}`}>
                        <Button className="w-full bg-primary hover:bg-primary/90">
                          <CheckCircle className="w-4 h-4 mr-2" />
                          Acessar Turma
                        </Button>
                      </Link> : status === 'pending' ? <Button disabled className="w-full" variant="secondary">
                        <Clock className="w-4 h-4 mr-2" />
                        Aguardando Aprovação
                      </Button> : status === 'rejected' ? <Button disabled className="w-full" variant="outline">
                        Solicitação Recusada
                      </Button> : <Button className="w-full bg-gradient-primary hover:opacity-90" onClick={() => {
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea placeholder="Conte um pouco sobre você ou por que deseja entrar nesta turma..." value={message} onChange={e => setMessage(e.target.value)} rows={3} />
            </div>
          </div>
          <DialogFooter>
